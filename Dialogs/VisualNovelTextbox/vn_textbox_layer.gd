...
@export var mirror_bubble: bool = false
...
func _apply_box_settings() -> void:
	var pico: Sprite2D = $Anchor/AnimationParent/Sizer/PicoBocadillo
	...
	if box_color_use_global:
		...
		pico.self_modulate = get_global_setting(&'bg_color', box_color_custom)
	else:
		dialog_text_panel.self_modulate = box_color_custom
		pico.self_modulate = box_color_custom

	var sizer: Control = %Sizer
	sizer.size = box_size
	sizer.position = box_size * Vector2(-0.5, 0)+Vector2(0, +box_margin_bottom)
	pico.position = box_size * Vector2(0, 1)
	
	pico.flip_h = false
	
	if mirror_bubble:
		pico.position = box_size * Vector2(1, 1)
		pico.flip_h = true
